...
def yearplot(
    ...
    by_day = data.resample('D').agg(how).reindex(
    ...
    by_day = pd.DataFrame({
        ...
        "week": ((by_day.index - by_day.index[0]).days // 7) + 1
    })
    ...
    for month in months_in_order:
        group = by_day[by_day.index.month == month]
        first = group.index.min()
        last = group.index.max()
        x0 = (first - by_day.index[0]).days // 7
        x1 = (last - by_day.index[0]).days // 7
