...
def generate_tasks(
    state: TaskState | None = None, number: int = 500
    ...
    states = (
        np.random.choice([state.name for state in TaskState], number)
        if state is None
...
async def fill_db(
    user_id: int, checklist_id: int, state: TaskState | None = None, number: int = 500
) -> None:
    ...
    df.to_sql(
        "tasks", cfg.db.build_connection_str(False), if_exists="append", index=False
    )
